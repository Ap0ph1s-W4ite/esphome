...
CONFIG_SCHEMA = cv.Schema(
    {
        ...
        cv.Optional(CONF_SENSITIVITY): number.number_schema(
            ...
            {
                cv.Optional(CONF_INITIAL_VALUE, default=5): cv.int_range(min=0, max=10),
                cv.Optional(CONF_MIN_VALUE, default=0): cv.int_,
                cv.Optional(CONF_MAX_VALUE, default=10): cv.int_,
            }
        ),
        cv.Optional(CONF_PERSISTENCE, default=0): number.number_schema(
            PIR233Persistence,
            ...
            {
                cv.Optional(CONF_INITIAL_VALUE, default=0): cv.int_range(min=0, max=10),
                cv.Optional(CONF_MIN_VALUE, default=0): cv.int_,
                cv.Optional(CONF_MAX_VALUE, default=10): cv.int_,
            }
        ),
